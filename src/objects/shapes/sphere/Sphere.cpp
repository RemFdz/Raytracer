...
bool Sphere::hit(const Rtx::Ray3D &ray, HitData_t &hitData, double tMin,
                 double tMax) {
    Math::Vec3 oc = _center - ray.getOrigin();
    ...
    outward_normal = (hitData.position - _center) / _radius;
    if (ray.getDirection().dot(outward_normal) > 0.0) {
        hitData.normal = -outward_normal;
        hitData.front_face = false;
    } else {
        hitData.normal = outward_normal;
        hitData.front_face = true;
    }
    return true;
}
